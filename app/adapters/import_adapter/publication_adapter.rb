...
module ImportAdapter
  class PublicationAdapter < BaseAdapter
    ...
    def purge
      @faculty.publications.each(&:destroy)
    end
    ...
    def build_author_list(row)
      ...
      keys = %i[ intellcont_auth_1_lname
                 intellcont_auth_1_fname
                 intellcont_auth_2_lname
                 intellcont_auth_2_fname
                 intellcont_auth_3_lname
                 intellcont_auth_3_fname
                 intellcont_auth_4_lname
                 intellcont_auth_4_fname
                 intellcont_auth_5_lname
                 intellcont_auth_5_fname
                 intellcont_auth_6_lname
                 intellcont_auth_6_fname ]
      ...
      (0..items.count - 1).step(2) do |pos|
        if items[pos][1].present? && items[pos + 1][1].present?
          authors << (items[pos][1] + ', ' + items[pos + 1][1]).strip
        end
      end
