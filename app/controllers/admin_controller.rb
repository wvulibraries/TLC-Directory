class AdminController < ApplicationController
  ...
  def index
    # @wvu_username = session[:cas_user]
    # @email = session[:cas_extra_attributes][:mail]
    # @last_name = session[:cas_extra_attributes][:sn]

    #redirect_to ads_path, success: "Welcome #{@wvu_username}!  You have been sucessfully logged in!"
  end

  private

  def check_permissions
    has_permission = User.where(wvu_username: session[:cas_user]).exists?
    if !has_permission
